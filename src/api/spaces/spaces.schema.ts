import { z } from 'zod';
import { TShade } from '../shades';

export const TAuthor = z.object({
  id: z.string(),
  name: z.string().optional(),
  picture: z.string().optional().nullable(),
});

export type Author = z.infer<typeof TAuthor>;

export const TVisibility = z.union([z.literal('PUBLIC'), z.literal('PRIVATE')]);

export type Visibility = z.infer<typeof TVisibility>;

export const TSpace = z.object({
  id: z.string(),
  name: z.string(),
  description: z.string(),
  picture: z.string().nullable(),
  cover: z.string().nullable(),
  shade: TShade.nullable(),
  tags: z.array(z.string()).optional(),
  membersCount: z.number(),
  privacy: TVisibility,
});

export type Space = z.infer<typeof TSpace>;

export const TSpaceDetails = z.object({
  id: z.string(),
  name: z.string(),
  picture: z.string().nullable(),
  cover: z.string().nullable(),
  shade: TShade.nullable(),
  description: z.string(),
  tags: z.array(z.string()).optional(),
  privacy: TVisibility,
  membersCount: z.number(),
  isUserJoined: z.boolean(),
  owner: TAuthor,
});

export const TInvitationSpace = z.object({
  id: z.string(),
  name: z.string(),
  picture: z.string().nullable(),
  shade: TShade.nullable(),
});

export type InvitationSpace = z.infer<typeof TInvitationSpace>;

const TInvitationStatus = z.union([z.literal('PENDING'), z.literal('ACCEPTED'), z.literal('DECLINED')]);

export type InvitationStatus = z.infer<typeof TInvitationStatus>;

export const TInvitation = z.object({
  id: z.string(),
  space: TInvitationSpace,
  status: TInvitationStatus,
  inviter: TAuthor,
});

export const TInvitationCode = z.object({
  id: z.string(),
  qrCode: z.string(),
  telegramUrl: z.string(),
});

export const TUserToInvite = z.object({
  id: z.string(),
  name: z.string(),
  picture: z.string().optional(),
  status: z.string(),
});
